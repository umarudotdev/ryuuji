...
pub async fn fetch_feed(
    ...
    feed: &TorrentFeed,
) -> Result<Vec<TorrentItem>, RyuujiError> {
    let response = client
        .get(&feed.url)
        .timeout(std::time::Duration::from_secs(30))
        .send()
        .await
        .map_err(|e| RyuujiError::Torrent(format!("fetch {}: {e}", feed.name)))?;

    let bytes = response
        .bytes()
        .await
        .map_err(|e| RyuujiError::Torrent(format!("read {}: {e}", feed.name)))?;

    let channel = rss::Channel::read_from(&bytes[..])
        .map_err(|e| RyuujiError::Torrent(format!("parse {}: {e}", feed.name)))?;
